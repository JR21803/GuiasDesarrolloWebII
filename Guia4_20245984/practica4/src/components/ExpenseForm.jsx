...
export const ExpenseForm = () => {
    ...
    const calculateTotal = () => {
        let currentExpenses = state.expenses.reduce((total, expense) => expense.amount + total, 0);

        if (state.editingId) {
            const originalExpense = state.expenses.find(exp => exp.id === state.editingId)

            if (originalExpense) {
                currentExpenses -= originalExpense.amount
            }
        }
        return currentExpenses + expense.amount
    }
